  ...
  useEffect(() => {
    if (examCode) {
      ...
      if (selectedExam) {
        ...
        const grantedUsersList = [];
        selectedExam.accessCode.forEach((access) => {
          if (access.code === examCode) {
            grantedUsersList.push(...access.users);
          }
        });
        setGrantedUsers(grantedUsersList);
      } else {
        setExamDetails(null);
        setGrantedUsers([]);
      }
                    ...
                    {grantedUsers.length > 0 && (
                      <>
                        ...
                        {grantedUsers.length > 1 && (
                          <tr>
                            <td colSpan="2" className="text-center py-0">
                              <button
